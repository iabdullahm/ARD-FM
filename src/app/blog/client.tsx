...
export default function BlogPage() {
  ...
  const handleNewsletterSubmit = async (e: React.FormEvent) => {
    ...
    try {
      const res = await fetch('https://formspree.io/f/mzbnblrj', {
        method: 'POST',
        headers: { 'Content-Type': 'application/json', Accept: 'application/json' },
        body: JSON.stringify({ email: newsletterEmail, source: 'Newsletter Subscription' }),
      });
        ...
        <div className="max-w-2xl mx-auto text-center">
          ...
          >
            ...
            <button
              ...
              className="px-6 py-3 bg-[#FF7A00] text-white font-semibold rounded-lg hover:bg-orange-600 transition disabled:opacity-50"
            >
          ...
          {newsletterStatus === 'error' && (
            <p className="mt-4 text-red-600 font-semibold">
              {lang === 'ar' ? 'تعذّر الاشتراك — تأكد من البريد وحاول مجدداً.' : 'Subscription failed — check your email and try again.'}
            </p>
          )}
        </div>
      </section>
